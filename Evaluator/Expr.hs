    ...
    maybeTimed (panic "Timed prefix in non-timed section") $ \ tockName fnName -> do
        createFunctionFrame n [] e $! \frameInfo -> do
            ...
            return $ do
                ...
                p' <- addTocker p
                let
                    tocker = POp PExternalChoice [PUnaryOp (PPrefix (tock tockName n)) procCall | n <- [1,2,4,8,16]]
...
timedCSPSyncSet prog = do
    maybeTimed prog $! \ tn _ -> do
        prog <- prog
        let set = S.fromList (tockValueList tn)
        return $! do
            VSet a <- prog
...
tock :: Name -> Int -> Event
tock tn n = UserEvent (tockValue tn n)

tockValue :: Name -> Int -> Value
tockValue tn n = VDot [VChannel tn, VInt n]

tockValue' :: Name -> Value
tockValue' tn = VDot [VChannel tn]

tockSet :: Name -> EventSet
tockSet tn = [tock tn n | n <- [1,2,4,8,16]]

tockValueList :: Name -> [Value]
tockValueList tn = [tockValue tn n | n <- [1,2,4,8,16]]

makeTocker :: Name -> Int -> Proc -> Proc
makeTocker tn tocks p
  | tocks == 0                   = p
  | tocks == 1                   = PUnaryOp (PPrefix (tock tn 1)) (makeTocker tn (0) p)
  | (tocks >= 2) && (tocks < 4)  = POp PExternalChoice [PUnaryOp (PPrefix (tock tn n)) (makeTocker tn (tocks - n) p) | n <- [1,2]]
  | (tocks >= 4) && (tocks < 8)  = POp PExternalChoice [PUnaryOp (PPrefix (tock tn n)) (makeTocker tn (tocks - n) p) | n <- [1,2,4]]
  | (tocks >= 8) && (tocks < 16) = POp PExternalChoice [PUnaryOp (PPrefix (tock tn n)) (makeTocker tn (tocks - n) p) | n <- [1,2,4,8]]
  | tocks >= 16                  = POp PExternalChoice [PUnaryOp (PPrefix (tock tn n)) (makeTocker tn (tocks - n) p) | n <- [1,2,4,8,16]]
