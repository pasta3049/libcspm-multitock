    ...
    let
        ...
        csp_tstop [tockVal@(VDot [VChannel tn])] =
            let
                ...
                proc = POp PExternalChoice [PUnaryOp (PPrefix (UserEvent (VDot [VChannel tn, VInt n]))) pc | n <- [1,2,4,8,16]]
        ...
        csp_wait [tockVal] =
            let
                ...
                mkTocker n
                  | n == 0               = tskip
                  | n == 1               = PUnaryOp (PPrefix (UserEvent (VDot [VChannel tn, VInt 1]))) (waitm 1)
                  | (n >= 2) && (n < 4)  = POp PExternalChoice [PUnaryOp (PPrefix (UserEvent (VDot [VChannel tn, VInt k]))) (waitm k) | k <- [1,2]]
                  | (n >= 4) && (n < 8)  = POp PExternalChoice [PUnaryOp (PPrefix (UserEvent (VDot [VChannel tn, VInt k]))) (waitm k) | k <- [1,2,4]]
                  | (n >= 8) && (n < 16) = POp PExternalChoice [PUnaryOp (PPrefix (UserEvent (VDot [VChannel tn, VInt k]))) (waitm k) | k <- [1,2,4,8]]
                  | (n >= 16)            = POp PExternalChoice [PUnaryOp (PPrefix (UserEvent (VDot [VChannel tn, VInt k]))) (waitm k) | k <- [1,2,4,8,16]]
                  where waitm k = let (VProc wm) = if (n >= k) then (waiter [VInt (n-k)]) else undefined in wm
